...
/**
 * Sanitize string to prevent XSS while preserving markdown content
 */
const sanitizeString = (str) => {
  if (!str) return "";
  // Only remove actual script tags and dangerous event handlers
  // Be more careful to preserve legitimate content
  return str
    .replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, "") // Remove script tags
    .replace(/on\w+\s*=\s*["'][^"']*["']/gi, "") // Remove event handlers like onclick
    .replace(/javascript:\s*[^"'\s]*/gi, ""); // Remove javascript: protocols
};
...
export const validateNote = (req, res, next) => {
  ...
  if (content && content.length > 5000000) {
    // Increased to 5MB
    errors.push("Content must be less than 5MB");
  }
  ...
  // Only sanitize title, leave content as-is to preserve markdown formatting
  req.body.title = title.trim();
  req.body.content = content; // Keep content exactly as provided
